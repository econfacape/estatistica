...
```{python}
# Definindo variável independente e a dependente
X = sm.add_constant(dados['Renda'])  # adiciona constante (intercepto)
y = dados['Consumo']

# Estimando o modelo pelo método de MQO
...
plt.plot(dados['Renda'], modelo.predict(X), color='red', label='Regressão')
plt.xlabel('Renda (US$)')
...
```{python}
nova_renda = 30
consumo_previsto = modelo.predict([1, nova_renda])  # 1 é o valor da constante
...
dados['y_estimado'] = modelo.predict(sm.add_constant(dados['Renda']))
